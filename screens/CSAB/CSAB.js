...
import data from "../../assets/data/csab.json";
...
const Csab = () => {
  ...
  console.log(year,name,selectedType,selectedCategory,selectedBranch,selectedRound,selectedGender);
  ...
  const institute=(type)=>{ 
    data.map((item)=>{
      if(item.Year==type){
        ...
        setRound(round=>[...round,item.Round]);
      }
    })
  }
  ...
  const BranchType=(type)=>{ 
    setBranch([]);
    data.map((item)=>{
      if(item.Institute_Name==type && item.Year==year && item.Round==selectedRound){
        // console.log(type);
        //console.log(item.Institute_Name);
  ...
  const GenderAndSeatType=(type)=>{ 
    ...
    setGender([]);
    data.map((item)=>{
      if(item.Institute_Name==name && item.Year==year && item.Round==selectedRound && item.Academic_Program==type ){
        // console.log(type);
        //console.log(item.Institute_Name);
  ...
  const result=()=>{ 
    setFinal((final)=>[]);
    data.map((item)=>{
      if(item.Year==year && item.Institute_Name==name && item.Institute_Type==selectedType && item.Seat_Type==selectedCategory && item.Academic_Program==selectedBranch && item.Round==selectedRound && item.Gender==selectedGender){
        setFinal([...final,item]);
        console.log(item);
      }
    })
  ...
  return (
    <ScrollView style={{flex:1}}>
      <SafeAreaView style={styles.safe}>
        <View >
          <Text style={{ alignSelf:'center', paddingHorizontal:5, fontSize: hp(4) ,fontWeight: 'bold' , color: '#56CB01',marginTop:50, marginBottom:15, }}>CSAB</Text>
        </View>
        <View>
        ...
        <View>
          ...
          textStyle={styles.textStyle} 
          defaultTextStyle={styles.deftext}
          defaultValue ='Select Round'
          renderButtonText={(value)=>setSelectedRound(value)}
          dropdownTextStyle={{fontSize:hp(2)}}
          options={round.filter(onlyUnique)}></ModalDropdown>
          <View style={{ position: "absolute", right: 20, top:20 }}><Text>▼</Text></View>
        </View>
        <View>
          <ModalDropdown style = {styles.drop} 
        ...
        <TouchableOpacity onPress={()=>result(year,name,selectedType,selectedCategory,selectedBranch,selectedRound,selectedGender)}>
